...
//CON COMPRESION
app.get('/info-comp', compression(), (req, res) => {
    res.json({
...
app.get('*', (req, res) => {
    ...
    logger.warn(`ruta ${method} ${url} no esta definida`)
    res.send(`ruta ${method} ${url} no esta definida`)
})
...
IO.on('connection', socket => {
    logger.info(`${socket.id} se ha conectado`);
